...
List breadth_first_search(Graph G, int x, int parent[]){
	...
	push_queue(&Q,x);
	parent[x]=0; //ptu dau tien khi duyet ko co cha
	...
	while(!empty_queue(Q)){
		...
		for(int i=1;i<=L.size;++i){
			...
			if(!marked[v]){
				...
				if(parent[v]==-1){ 
					parent[v]=u; //neu v chua co cha thi cha cua v la u
				}
			}
		}
	}
...
int main(int argc, char *argv[]){
	...
	int parent[MAX_VERTICES]; //khoi tao mang luu tru cha cua cac dinh 
	for(int i=1;i<=G.n;++i){ 
		marked_bfs[i]=0;
		parent[i]=-1; //ban dau cac dinh ko co cha
	}
	
	for(int i=1;i<=G.n;++i){ //lan luot luot qua thu tu cac dinh trong do thi
		if(!marked_bfs[i]){ //neu dinh i chua duyet
			List L = breadth_first_search(G,i, parent); //thi se tao ds duyet tu dinh i
